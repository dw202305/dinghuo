...
- 状态字段：`status` TINYINT，含义在注释中说明
- 金额字段：`BIGINT` 存"分"，字段名后缀 `_cent`（如 `unit_price_cent`、`total_amount_cent`）
- 尺寸字段：`DECIMAL(6,1)` 存厘米
- 面积字段：`DECIMAL(12,4)` 存平方米
- 轨道长度字段：`DECIMAL(6,2)` 存米（2位小数精度到厘米）
- **禁止使用 FLOAT / DOUBLE 类型**
...
### 1.2.1 软删除策略
- **使用软删除的表**：lj_store, lj_store_contact, lj_fabric, lj_track, lj_accessory, lj_admin
- **不使用软删除的表**：lj_payment, lj_after_sale, lj_invoice_request, lj_order, lj_order_item, lj_inventory_log, lj_operation_log, lj_customer_balance_account, lj_customer_balance_transaction, lj_recharge_order 等财务/流水/日志类表
- 财务相关表用 status 字段管理关闭/作废状态，不做物理或逻辑删除

### 1.3 索引约定
- 唯一索引：`uk_表名_字段名`
- 普通索引：`idx_表名_字段名`
- 外键索引：`fk_表名_字段名`（逻辑外键，不建物理外键）
...
CREATE TABLE `lj_after_sale` (
  ...
  `order_id` BIGINT UNSIGNED NOT NULL COMMENT '订单ID',
  `order_no` VARCHAR(50) NOT NULL COMMENT '订单号',
  `store_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '门店ID（v1.3.2 补建，来源 lj_order.transaction_id）',
  ...
  `close_reason` VARCHAR(500) DEFAULT NULL COMMENT '关闭原因（v1.3.2 补建）',
  ...
  `handled_at` DATETIME DEFAULT NULL COMMENT '处理时间（v1.3.2 补建）',
  ...
  KEY `idx_after_sale_store_id` (`store_id`),
...
### 3.5 储值单状态 recharge_order.status
| 值 | 含义 |
|---|---|
| 1 | 待支付 |
| 2 | 支付中 |
...
```
lj_store (门店) ──┬── lj_store_contact (联系人)
                  ├── lj_store_address (收货地址)
                  ├── lj_store_inventory (库存)
                  ├── lj_order (订单) ──── lj_order_item (窗帘明细)
                  │                    └─── lj_payment (支付)
                  │                    └─── lj_after_sale (售后)
                  │                    └─── lj_invoice_request (发票)
                  ├── lj_customer_balance_account (资金账户)
                  │         └─── lj_customer_balance_transaction (资金流水)
                  │         └─── lj_recharge_order (储值订单)
                  └── lj_account_customer ──── lj_account (登录账号)

lj_partner (合伙人) ──┬── lj_store (下属门店)
                      └── lj_customer_balance_account (资金账户)

lj_payment ──── lj_customer_balance_transaction (通过 balance_transaction_id 关联)

lj_sales (销售) ──── lj_customer_attribution_history (归属历史)

lj_fabric (面料) ──── lj_fabric_supplier_mapping ──── lj_fabric_supplier (供应商)

lj_track (轨道)
lj_accessory (配件)

lj_inventory_log (库存流水) ──── lj_store_inventory
lj_operation_log (操作日志) ──── 各业务模块
```
...
1. **订单列表查询**：`order_status` + `created_at` 复合索引
2. **门店订单查询**：`transaction_type` + `transaction_id` + `order_status`
3. **面料搜索**：`fabric_no` + `name` + `series` 全文索引（或单独搜索服务）
4. **库存流水**：`store_id` + `created_at` 按时间范围查询
5. **操作日志**：`target_type` + `target_id` + `created_at` 按目标追溯
6. **资金流水查询**：`account_id` + `created_at` 按账户时间范围查询
7. **余额支付对账**：`payment_channel` + `pay_status` + `created_at`

---

## 六、后续扩展预留

1. **CRM 对接**：`crm_customer_id`、`crm_opportunity_id` 字段已预留
2. **多语言**：所有文本字段使用 `utf8mb4`
3. **分库分表**：订单表按 `store_id` 或 `created_at` 分片
4. **读写分离**：报表查询走从库
5. **缓存策略**：面料、轨道、配件等基础数据 Redis 缓存
6. **多币种**：`lj_customer_balance_account` 已预留 `currency` 字段
...
2. 面积字段使用 `DECIMAL(12,4)` 存平方米，尺寸字段使用 `DECIMAL(6,1)` 存厘米
3. 轨道长度字段使用 `DECIMAL(6,2)` 存米，精度到厘米
4. 禁止使用 FLOAT / DOUBLE 类型
5. 所有关联使用逻辑外键，不建物理外键，便于后续分库分表
6. 软删除字段 `deleted_at` 已预留，查询时需过滤 `IS NULL`
